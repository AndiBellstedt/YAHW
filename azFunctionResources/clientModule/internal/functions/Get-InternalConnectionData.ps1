...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'YAHW.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'YAHW.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'YAHW.Client.ProtectedToken'
